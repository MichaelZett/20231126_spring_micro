...
class CopyRestController {
    ...
    @PostMapping("/")
    public ResponseEntity<String> add(@Valid @RequestBody CopyAddTo copyTo) {
        copyService.addCopies(copyTo);
        return ResponseEntity.ok("Success");
    }

    @PostMapping("/out")
    public ResponseEntity<String> timeout(@Valid @RequestBody CopyAddTo copyTo) {
        // use to trigger timeout
        try {
